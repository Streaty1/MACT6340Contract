...
error JKSCreativeNFTContract_MaxSupplyReached();
error JKSCreativeNFTContract_ValueNotEqualPrice();
error JKSCreativeNFTContract_WrongAvenueForThisTransaction();

/// @custom:security-contact jstreaty@gmail.com
contract JKSCreativeNFTContract is 
ERC721, 
ERC721Enumerable, 
ERC721URIStorage, 
ERC721Burnable, 
...
ReentrancyGuard 
{
    ...
    //     ERC721("JKSCreativeNFTContract", "JKS")
        ...
        Ownable(msg.sender)
    {
        ...
        _royaltyArtist = _royaltyArtist;
        _setDefaultRoyalty(_royaltyArtist, _royaltyBasis);
        i_owner = msg.sender;
    }

    receive() external payable {
        revert JKSCreativeNFTContract_WrongAvenueForThisTransaction();
        }
    ...
    revert JKSCreativeNFTContract_MaxSupplyReached();
    }
    // make sure there us the right amount of money
    if (msg.value != i_mint_price) {
    revert JKSCreativeNFTContract_ValueNotEqualPrice();
    }
    _tokenIdCounter++;
