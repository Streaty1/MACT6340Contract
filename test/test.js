...
describe("JKSCreativeNFTContract", async function () {
  let JKSCreativeNFTContractFactory;
  let JKSCreativeNFTContract;
  let args = {
  ...
  this.beforeEach(async function () {
    JKSCreativeNFTContractFactory = await ethers.getContractFactory(
      "JKSCreativeNFTContract"
    );
    JKSCreativeNFTContract = await JKSCreativeNFTContractFactory.deploy(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
    ...
    await JKSCreativeNFTContract.waitForDeployment(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
  ...
  describe("construction and initialization", async function () {
    ...
    it("should be named JKSCreativeNFTContract", async function () {
      const expectedValue = "JKSCreativeNFTContract";
      const currentValue = await JKSCreativeNFTContract.name();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should be have symbol JKS", async function () {
      const expectedValue = "JKS";
      const currentValue = await JKSCreativeNFTContract.symbol();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a mint price set when constructed", async function () {
      const expectedValue = args.mint_price;
      const currentValue = await JKSCreativeNFTContract.getMintPrice();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a max token supply set when constructed", async function () {
      const expectedValue = args.max_tokens;
      const currentValue = await JKSCreativeNFTContract.getMaxSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });

    it("should have a base URI set when constructed", async function () {
      const expectedValue = args.base_uri;
      const currentValue = await JKSCreativeNFTContract.getBaseURI();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should set roylaty artist when constructed", async function () {
      ...
      const currentValue = await JKSCreativeNFTContract.royaltyInfo(
        1,
  ...
  describe("receive function", async function () {
    ...
    it("should be called and revert if called from low-level transaction", async function () {
      let contractAddress = await JKSCreativeNFTContract.getAddress();
  ...
  describe("mintTo function", async function () {
    ...
    it("should revert if called with no ether", async function () {
      const [owner, artist, buyer] = await ethers.getSigners();
      expect(
        JKSCreativeNFTContract.connect(buyer).mintTo(args.base_uri, {
    ...
    it("should have a totalSupply = 1 with after first mint", async function () {
      ...
      const mint1 = await JKSCreativeNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint1.wait(1);
      const currentValue = await JKSCreativeNFTContract.totalSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should increase the totalSupply by 1 with with each mint", async function () {
      ...
      mint1.wait(1);
      const mint2 = await JKSCreativeNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint2.wait(1);
    ...
    it("should emit an event when minting is completed", async function () {
      ...
      expect(
        ...
        .to.emit(JKSCreativeNFTContract, "MintingCompleted")
        .withArgs(1, owner);
    });
    ...
    it("should have set the token uri during minting function", async function () {
      ...
      mint1.wait(1);
      const currentValue = await JKSCreativeNFTContract.tokenURI(1);
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should emit an event after funds are distributed during mint", async function () {
      ...
      const uriString = "someString";
      expect(
        JKSCreativeNFTContract.connect(buyer).mintTo(uriString, {
          value: args.mint_price,
        })
      )
        .to.emit(JKSCreativeNFTContract, "FundsDistributed")
