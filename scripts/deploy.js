...
async function main() {
  ...
  const JKSCreativeNFTContractFactory = await ethers.getContractFactory(
    "JKSCreativeNFTContract"
  );
  // deploy
  const JKSCreativeNFTContract = await JKSCreativeNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await JKSCreativeNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await JKSCreativeNFTContract.mintTo(ipfs[i], {
